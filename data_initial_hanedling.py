...
class Scaler:
    def __init__(self):
        self.features_to_normalize = [1, 10, 11, 12, 13, 14, 16, 17, 25, 27, 30, 33]
        self.features_to_standartize = [2, 3, 4, 6, 15, 18, 19, 20, 22, 23, 24, 26, 29, 31, 32]
        self.dict = {}
    ...
    def scale(self, data: DataFrame):
        for column_index in self.features_to_normalize:
            self.normalize(column_index, data)
        for column_index in self.features_to_standartize:
            self.standartize(column_index, data)

    def standartize(self, column_index, data: DataFrame):
        feature_name = data.columns.values[column_index]
        dict_val = self.dict.get(column_index)
        mean = dict_val[0]
        std = dict_val[1]
        if std == 0:
            std = 1
    ...
    def normalize(self, column_index, data: DataFrame):
        feature_name = data.columns.values[column_index]
        dict_val = self.dict.get(column_index)
        max = dict_val[0]
        min = dict_val[1]
        if min == max:
            data[feature_name] = (data[feature_name] - min)
        else:
